        ...
        <div class="flex h-screen antialiased text-gray-900 bg-gray-100 dark:bg-dark dark:text-light">
            ...
            <div class="flex-1 h-full overflow-x-hidden overflow-y-auto">
                ...
                <main>
                    <div class="grid grid-cols-1 p-4 space-y-8 lg:gap-8 lg:space-y-0 lg:grid-cols-4">
                        <div class="col-span-2 bg-white rounded-md dark:bg-darker" x-data="{ isOn: false }">
                            ...
                            <table>
                                ...
                                <tbody>
                                    <?php
                    // กำหนดอาร์เรย์สำหรับเก็บจำนวนของแต่ละกลุ่มอายุ
                    $ageCounts = array_fill(10, 80, 0); // กำหนดค่าเริ่มต้นให้มีค่า 0 สำหรับอายุ 10 - 80 ปี

                    foreach ($dataArray['result'] as $survey) {
                        // ตรวจสอบว่าฟิลด์อายุตรงตามเงื่อนไขหรือไม่
                        if (!empty($survey['age']) && $survey['age'] !== '' && $survey['age'] !== 'none') {
                            // ตรวจสอบว่าอายุอยู่ในช่วง 10 - 99 และเพิ่มค่านับให้กับกลุ่มอายุที่เกี่ยวข้อง
                            $age = intval($survey['age']); // แปลงอายุเป็นจำนวนเต็มเพื่อเปรียบเทียบ
                            if ($age >= 10 && $age <= 99) {
                                $ageCounts[$age]++;
                            }
                        }
                    ...
                    // แสดงจำนวนของแต่ละกลุ่มอายุ
                    for ($age = 10;  $age <= 80; $age++) {
                        if ($ageCounts[$age] > 0) { // ตรวจสอบว่ามีจำนวนมากกว่า 0
                            echo '<tr>';
                            echo '<td>' . $age . ' ปี</td>';
